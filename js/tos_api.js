function tos_api() {
    ...
    self.event = {
        get: async function() {
            ...
            if(!events) {
                console.log("開始向 API 請求活動資料");
                var response = await fetch("https://tos-api.pascaltheelf.workers.dev/v1/event/get").then(r=>r.json());
                if(response.info.success) {
                    console.log("成功向 API 請求活動資料", response);
                    if(self.settings.cache.allow) {
    ...
    self.card = {
        get: async function(id) {
            ...
            if(que.length > 0) {
                console.log("開始向 API 請求卡片資料", que);
                var response = await fetch("https://tos-api.pascaltheelf.workers.dev/v1/card/info?id="+que.join(",")).then(r=>r.json());
                if(response.info.success) {
                    console.log("成功向 API 請求卡片資料", response);
                    if(self.settings.cache.allow) {
                        console.log("已啟用快取功能，寫入快取資料");
    ...
    self.stage = {
        get: async function(name) {
            ...
            if(!stage) {
                ...
                var response = await fetch("https://tos-api.pascaltheelf.workers.dev/v1/stage/info?name="+name).then(r=>r.json());
                if(response.info.success) {
